// nGrinder 부하 스크립트 — [랭킹] 전국 랭킹 탐색
//   홈화면 → 전국 선택 → /trend/nationwide → 가게 상세
//
// 구조 메모:
//   - home()       : 좌표 → mock(coord2regioncode) → 구_동 → BE /trend/select (현위치 홈 피드)
//   - nationwide() : GET /trend/nationwide → 응답 hotStores[] storeId 수집
//   - storeDetail(): 수집된 storeId 앞 최대 3개 → /stores/{id}
//
...
class TestRunner {
	...
	public static GTest tNationwide
	...
	public static void beforeProcess() {
		...
		tNationwide = new GTest(2, "02 nationwide")
		tStore      = new GTest(3, "03 store-detail")
	...
	public void beforeThread() {
		...
		tNationwide.record(this, "nationwide")
	...
	// ── 스레드 로컬 상태 ──────────────────────────────────────────────────────
	private List nationwideStoreIds = new ArrayList()
	...
	// ──────────────────────────────────────────────────────────────────────────
	// 02 nationwide: GET /trend/nationwide → hotStores[] storeId 수집
	// ──────────────────────────────────────────────────────────────────────────
	public void nationwide() {
		nationwideStoreIds.clear()
		NVPair[] params = new NVPair[0]
		HTTPResponse r  = request.GET(BASE + "/trend/nationwide", params, authHeaders())
		assertThat(r.statusCode, is(200))
		try {
			def parsed    = new JsonSlurper().parseText(r.getText())
			def hotStores = parsed?.data?.hotStores
			if (hotStores != null) {
				for (s in hotStores) {
					if (s?.storeId != null) nationwideStoreIds.add(s.storeId as long)
				}
			}
		} catch (Exception e) {
			grinder.logger.warn("nationwide parse fail " + e.message)
		}
		grinder.logger.info("nationwide stores=" + nationwideStoreIds.size())
	}

	// ──────────────────────────────────────────────────────────────────────────
	// 03 storeDetail: 전국 랭킹 결과 앞 최대 3개 가게 상세 진입
	// ──────────────────────────────────────────────────────────────────────────
	public void storeDetail() {
		int count = Math.min(3, nationwideStoreIds.size())
		for (int i = 0; i < count; i++) {
			long sid   = (long) nationwideStoreIds.get(i)
